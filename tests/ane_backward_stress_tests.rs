...
use rustane::training::{
    ANEGradientBuffer, CrossEntropyLoss, LossFn, Model, 
...
fn test_stress_gradient_accumulation() {
    ...
    for i in 0..num_accumulations {
        ...
        if i % 10 == 0 {
            println!("Accumulation {}: count={}, max_grad={:.4e}", 
                i, buffer.accumulation_count(), buffer.max_abs_gradient());
        }
    }
...
fn test_stress_rapid_reset_and_accumulate() {
    ...
    for _ in 0..100 {
        buffer.reset();
        buffer.accumulate(&vec![0.01f32; config.param_count()]).unwrap();
    }
...
fn test_stress_gradient_precision() {
    ...
    assert!(relative_error < 0.01, "Precision loss too high: {}", relative_error);
}
...
fn test_stress_parameter_update_stability() {
    ...
    for i in 0..10 {
        ...
        assert!(current_params.iter().all(|p| p.is_finite()), 
            "Parameters became non-finite at iteration {}", i);
