...
fn main() {
    ...
    for (name, config) in configs {
        println!("Configuration: {} ({}M parameters)", name, config.param_count() / 1_000_000);
        println!("  dim={}, seq_len={}, hidden_dim={}, n_heads={}, n_layers={}, vocab_size={}",
            config.dim, config.seq_len, config.hidden_dim, config.n_heads, config.n_layers, config.vocab_size);
        ...
        println!("  ANE backward: {:.2}ms (avg of 5 runs)", ane_avg.as_secs_f64() * 1000.0);
        println!("  CPU backward: {:.2}ms (avg of 5 runs)", cpu_avg.as_secs_f64() * 1000.0);
        ...
        if ane_avg < cpu_avg {
            ...
            println!("  Note: ANE is {:.2}x slower (likely due to compilation overhead)", slowdown);
        }
        println!();
    }
