//! SDPA backward pass 1 MIL compile + eval test - single output versions.
//!
//! Usage: ./test_backward_sdpa_bwd1
//! Tests bwd_sdpa_bwd1_dvf_mil, bwd_sdpa_bwd1_dpf_mil, bwd_sdpa_bwd1_pf_mil compilation and evaluation on ANE.
...
fn main() {
    ...
    eprintln!(
        "Testing SDPA backward pass 1: dim={} seq={} heads={} head_dim={}",
    ...
    let mask_data: Vec<f32> = (0..seq * seq)
        .map(|i| if i % seq >= i / seq { 0.0 } else { -1000.0 }) // causal mask
        .collect();
    ...
    eprintln!(
        "Weight blobs created: Wot={} bytes, mask={} bytes",
        wot.len(),
        mask.len()
    );
    ...
    eprintln!("\n=== Testing dvf computation ===");
    {
        ...
        eprintln!("=== DVF MIL (first 50 lines) ===");
        ...
        // Extract weight info from request
    ...
    // === Test dpf (dprobs gradient) ===
    eprintln!("\n=== Testing dpf computation ===");
    {
        let mil = rustane::mil::bwd_sdpa_bwd1_dpf_mil(seq, dim, heads, head_dim);
        let req =
            rustane::mil::bwd_sdpa_bwd1_dpf_compile_request(seq, dim, heads, head_dim, &wot, &mask);
    ...
    eprintln!("\n=== Testing pf computation ===");
    {
        ...
        match exec {
            Ok(mut executor) => {
                ...
                let input_elements = heads * seq * seq;
                let input: Vec<u8> = (0..input_elements)
                    .map(|i| {
                        let v: f32 = ((i % 100) as f32) * 0.01;
                        v.to_le_bytes()
    ...
    if all_ok {
        println!("\nOK - All SDPA backward pass 1 tests passed!");
    } else {
