...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (dim, seq) in configs {
        ...
        // ANE forward
        let mut exec = ANECompiler::new().compile_multi(
            &mil,
            &[],
            &[],
            &[],
        ...
        println!(
            "ANE: {:.0} steps/sec ({:.0}μs/step)",
            ane_tput,
            ane_elapsed.as_micros() as f64 / num_steps as f64
        );
        println!(
            "CPU: {:.0} steps/sec ({:.0}μs/step)",
            cpu_tput,
            cpu_elapsed.as_micros() as f64 / num_steps as f64
        );
        println!(
            "Speedup: {:.2}x {}",
            speedup,
